...
try:
    ...
    # For older gym versions that don't have deprecation
    def deprecation(msg):
        warn(msg)
...
try:
    ...
    # For older gym versions that don't have gym.vector
    def batch_space(space, n=1):
        """Create a batched version of a space for vectorized environments."""
        import gym.spaces as spaces
        import numpy as np
        
        if isinstance(space, spaces.Discrete):
            return spaces.MultiDiscrete([space.n] * n)
            ...
            return spaces.Tuple([batch_space(subspace, n) for subspace in space.spaces])
        elif isinstance(space, spaces.Dict):
            return spaces.Dict({key: batch_space(subspace, n) for key, subspace in space.spaces.items()})
        else:
            # For other space types, return a tuple of the original space
