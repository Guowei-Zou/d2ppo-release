...
Enhanced with Dispersive Loss for improved representation learning.
...
class PPODiffusion(VPGDiffusion):
    ...
    def __init__(
        ...
        norm_adv: bool = True,
        # Dispersive Loss in RL fine-tuning
        use_dispersive_loss_rl: bool = False,
        dispersive_loss_weight_rl: float = 0.1,  # Smaller weight for RL fine-tuning
        **kwargs,
    ):
        ...
        # Dispersive Loss for RL fine-tuning
        self.use_dispersive_loss_rl = use_dispersive_loss_rl
        self.dispersive_loss_weight_rl = dispersive_loss_weight_rl
        
        if self.use_dispersive_loss_rl:
            log.info(f"Enabled Dispersive Loss for RL fine-tuning with weight {dispersive_loss_weight_rl}")
    ...
    def loss(
        ...
        """
        Enhanced PPO loss with optional Dispersive Loss for RL fine-tuning
        ...
        # Dispersive Loss for RL fine-tuning
        dispersive_loss_rl = 0
        if self.use_dispersive_loss_rl:
            # Sample some actions to compute dispersive loss on intermediate representations
            B = obs['state'].size(0) if 'state' in obs else obs['rgb'].size(0)
            
            # Use a random timestep for dispersive loss computation
            t_disp = torch.randint(0, self.ft_denoising_steps, (B,), device=self.device).long()
            
            # Create noisy actions from the chain
            x_noisy = chains[:, -1]  # Use the final actions from the chain
            noise = torch.randn_like(x_noisy)
            x_t = self.q_sample(x_noisy, t_disp, noise)
            
            # Compute dispersive loss on fine-tuned model
            dispersive_loss_rl = self.get_intermediate_representations(
                x_t, t_disp, obs, use_finetuned=True
            )
        ...
        # Add dispersive loss for RL fine-tuning
        if self.use_dispersive_loss_rl:
            pg_loss = pg_loss + self.dispersive_loss_weight_rl * dispersive_loss_rl
        ...
        if self.use_dispersive_loss_rl:
            return (
                ...
                bc_loss,
                eta.mean().item(),
                dispersive_loss_rl.item() if isinstance(dispersive_loss_rl, torch.Tensor) else dispersive_loss_rl,
            )
        else:
            return (
                pg_loss,
    ...
    def get_intermediate_representations(self, x, t, cond, use_finetuned=True):
        """
        Enhanced version that can use either base or fine-tuned model for dispersive loss.
        """
        representations = []
        ...
        target_model = self.actor_ft if use_finetuned else self.actor
        
        # Register hooks
        hooks = []
        if hasattr(target_model, 'encoder') or hasattr(target_model, 'layers'):
            layers = getattr(target_model, 'encoder', getattr(target_model, 'layers', []))
            ...
            modules = list(target_model.modules())
            if len(modules) > 1:
